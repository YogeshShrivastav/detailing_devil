...
@Component({
  ...
  templateUrl: './consumer-lead-detail.component.html'
})
export class ConsumerLeadDetailComponent implements OnInit {
  form: any = {};
  data: any = [];
  ...
  constructor(public db: DatabaseService, private route: ActivatedRoute, private router: Router, public ses: SessionStorage,
    ...
    //   this.db.get_rqst( '', 'consumer_leads/locations/get/' + this.lead.country)
    //     .subscribe(data => {
    //       this.data = data;
    //       this.loading_list = true;
    
    //     if(this.db.datauser.franchise_id){
    //       this.locations = [{'id': this.db.datauser.location_id, 'location_name': this.db.datauser.location_name }];
    
    //     }else{
    //       this.locations = this.data.data.locations;
    //     }
    
    //     },err => {  this.dialog.retry().then((result) => { this.getLocations(); });   });
    // }
    ...
    getFranchiseList(){  
      ...
      .subscribe(data => {  
        ...
        if(this.db.datauser.franchise_id){
          this.franchise_list = [{'id':this.db.datauser.franchise_id, 'location_id': this.db.datauser.location_id, 'name': this.db.datauser.first_name, 'location_name': this.db.datauser.location_name}]
        }else{
    ...
    jobcard_forward(){
      ...
      if(this.lead.state == '')
      {
        this.dialog.warning('Please Assign State!'); 
      }
      else{
          if(this.lead.franchise_id){
            this.router.navigate(['/addjobcard/'+this.db.crypto(this.lead_id) +'/0/'+this.db.crypto(this.lead.franchise_id)]); 
          }else{
    ...
    updateLead() {
      ...
      this.db.post_rqst(this.formData, 'consumer_leads/update').subscribe(data => {
        this.loading_list = true;
        console.log(data);
