...
@Component({
  selector: 'app-franchise-followup',
  templateUrl: './franchise-followup.component.html'
})
export class FranchiseFollowupComponent implements OnInit {
  franchise_id;
  loading_list = false;
  ...
  getFollowUpList(status:any = 'pending') {
    ...
    this.status = status;
    this.db.post_rqst(  {'franchise_id':this.franchise_id,'login':this.db.datauser}, 'franchises/fanchises_getFollowUps?page=' + this.current_page + '&status=' + status + '&s=' + this.search)
    .subscribe(d => {
      console.log(d);
