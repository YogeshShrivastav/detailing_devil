...
@Component({
  selector: 'app-jobcard-payment-list',
  templateUrl: './jobcard-payment-list.component.html',
})
export class JobcardPaymentListComponent implements OnInit {
  ...
  constructor(
    ...
    private router: Router,
     ...
     @Inject(MAT_DIALOG_DATA) public lead_data: any,
      public dialogRef: MatDialogRef<JobcardPaymentListComponent>) 
      {
      this.franchise_id = lead_data.franchise_id; this.invoice_id = lead_data.invoice_id;
  ...
  receive_payments() {
    ...
    this.formData.date_created = this.formData.date_created   ? this.db.pickerFormat(this.formData.date_created) : '';
...
console.log(this.formData );
    ...
    this.db.insert_rqst( {'data': this.formData } , 'jobcard/jobcard_payment_receiving')
      .subscribe(data => {
  ...
  getPendingPayment() {
    ...
    this.db.post_rqst( {'data':this.formData}, 'jobcard/getJobcardPendingPayment')
      .subscribe(data => {
        console.log(data);
    this.sendingData = false;
      ...
      },err => {  this.dialog.retry().then((result) => { this.getPendingPayment();  }); });
  }
