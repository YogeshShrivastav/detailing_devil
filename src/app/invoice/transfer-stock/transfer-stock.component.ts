...
export class TransferStockComponent implements OnInit {
  ...
  constructor(public db: DatabaseService,
    ...
    AddItem(form)
    {
      ...
      this.data.transfer_qty = this.data.qty;
      ...
      if(itemFoundIndex !== null) {
        ...
        this.invoiceItemList[itemFoundIndex].transfer_qty = parseInt(this.invoiceItemList[itemFoundIndex].transfer_qty) + parseInt(this.data.transfer_qty);
    ...
    update_qty(){
      ...
      for (let j = 0; j < this.invoiceItemList.length; j++)
      {
        ...
        if(this.invoiceItemList[j].sale_qty < this.invoiceItemList[j].transfer_qty){
          this.grater_stock++;
        }
        
        
        if( ( this.invoiceItemList[j].transfer_qty > this.invoiceItemList[j].qty ) &&  this.invoiceItemList[j].id ){
          this.required_qty++;
        }
    ...
    submit_sales_invoice()
    {
      ...
      this.db.insert_rqst( {'stock':this.invoiceData} , 'sales/stockTxransfer')
      .subscribe((result: any) => {
