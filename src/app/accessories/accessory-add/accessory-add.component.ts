...
export class AccessoryAddComponent implements OnInit {
  ...
  storeUnitData(measurement, measurement_value, sale_price, purchase_price, description:any='') {
    ...
    if(measurement && measurement_value && (sale_price || purchase_price)) {
      ...
      }  else { this.dialog.error( 'Please Enter a valid integer for purchase price'); }

   } else { this.dialog.error( 'Please add Measurement and purchase to add unit data'); }
  ...
  saveProduct() {
    if(this.form.measurement == null) {
      if(this.form.attrtype == null) {
          ...
          this.formData.category = 'Accessory';
